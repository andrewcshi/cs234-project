...
import os
from env import ObstacleEnv  # Changed from CrowdEnv to ObstacleEnv
...
class Network(nn.Module):
    def __init__(self, robot_dim, obstacle_dim, lstm_hidden_dim, num_actions):
        ...
        # LSTM processes obstacle states (previously human states)
    ...
    def forward(self, state):
        ...
        obstacle_state = state[:, :, self.robot_dim:]  # Changed from human_state to obstacle_state

        h0 = torch.zeros(1, size[0], self.lstm_hidden_dim)
        c0 = torch.zeros(1, size[0], self.lstm_hidden_dim)
        output, (hn, cn) = self.lstm(obstacle_state, (h0, c0))
        hn = hn.squeeze(0)
...
class DQN:
    def __init__(self, model_path, env, lr, batch_size, gamma, eps_decay, eps_start, eps_end, initial_memory, memory_size):
        ...
        self.num_actions = 80  # Discrete action space size
        self.robot_dim = 6     # Robot state dimension
        self.obstacle_dim = 7  # Obstacle state dimension (changed from human_dim)
    ...
    def make_model(self):
        model = Network(self.robot_dim, self.obstacle_dim, self.lstm_hidden_dim, self.num_actions)
        return model
    ...
    def train(self, num_episodes=2000):
        ...
        for episode in range(num_episodes):
            # Reset environment with obstacles instead of humans
            obs = env.reset(obstacle_num=5, layout="circle", test_phase=False, counter=None)
            ...
            while True:
                ...
                # Periodic model saving and plotting
                ...
                if max(len(self.replay_buffer), len(self.replay_buffer_b)) >= self.initial_memory and not self.first:
                    ...
                    print(f"Main buffer size: {len(self.replay_buffer)}")
                    print(f"Collision buffer size: {len(self.replay_buffer_b)}")
                    self.first = True
                
                # Episode termination
                if terminal:
                    rewards_list.append(reward_for_episode)
                    # Store experiences based on episode outcome
                    if info == "collision":  # Changed from "collide" to "collision" to match environment
                        self.replay_buffer_b += ep_buffer
                    else:
                        self.replay_buffer += ep_buffer
                    print(f"Episode: {episode} done, Reward: {reward_for_episode}, Status: {info}")
                    break
...
def plot_stats(frame_idx, rewards, losses, step):
    ...
    plt.subplot(131)
    plt.title(f'Total frames {frame_idx}. Avg reward over last 10 episodes: {np.mean(rewards[-10:]) if rewards else 0}')
    plt.plot(rewards)
    plt.subplot(132)
    plt.title('Loss')
    plt.plot(losses)
...
if __name__ == "__main__":
    # Initialize obstacle environment instead of crowd environment
    env = ObstacleEnv()
    ...
    print('Starting training with obstacle environment')
