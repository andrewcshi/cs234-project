...
class CrowdEnv(object):
    ...
    def convert_coord(self, obs):
        assert len(obs) == 6
        # Use torch.tensor with dtype=torch.float32 to ensure consistent type
        robot_state = torch.tensor(obs[0], dtype=torch.float32)
        
        # Convert human_state to numpy array first, then to tensor with float32 dtype
        human_state = torch.tensor(np.array(obs[1:]), dtype=torch.float32)
        
        # Make sure human_state has the right shape
        assert human_state.shape[0] == 5
        assert human_state.shape[1] == 5
        
        dx = robot_state[5] - robot_state[0]
        dy = robot_state[6] - robot_state[1]
        dg = torch.tensor(norm((dx.item(), dy.item())), dtype=torch.float32).expand(5, 1)
        ...
        # Calculate distances between robot and humans
        distances = []
        for i in range(human_state.shape[0]):
            dist = norm((human_state[i, 0].item() - robot_state[0].item(), 
                        human_state[i, 1].item() - robot_state[1].item()))
            distances.append(dist)
        
        da = torch.tensor(distances, dtype=torch.float32).unsqueeze(1)
        
        new_state = torch.cat([dg, rot_expand, vx, vy, v_pref, radius, 
                            px_human, py_human, vx_human, vy_human, 
                            radius_human, da, radius_sum], dim=1).unsqueeze(0)
